  ...
  return (
    <div className="h-[100dvh] flex flex-col items-center px-4 py-3 safe-area-inset relative overflow-hidden">
      ...
      <div className="w-full max-w-sm flex flex-col items-center flex-1 min-h-0">
        {/* Team name badge + player */}
        <div className={`px-3 py-1 rounded-full text-xs font-display font-semibold mb-1 shrink-0 ${
        ...
        <div className="flex items-center gap-3 mt-2 shrink-0">
          <div className={`relative w-[5rem] h-[5rem] ${isBlinking ? "animate-countdown-pulse" : ""}`}>
            <svg className="w-full h-full -rotate-90" viewBox="0 0 120 120">
              <circle cx="60" cy="60" r="48" fill="none" stroke="hsl(var(--muted))" strokeWidth="16" />
              <circle
            ...
            <span className={`absolute inset-0 flex items-center justify-center text-xl font-display font-bold transition-colors duration-300 ${isUrgent ? "text-destructive" : ""}`}>
              {timeLeft}
            </span>
          </div>
